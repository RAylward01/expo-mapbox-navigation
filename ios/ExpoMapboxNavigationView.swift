...
class ExpoMapboxNavigationViewController: UIViewController {
    ...
    private func setupThrottledLocationUpdates() {
        // Use navigation.locationMatching instead of tripSession.location for MapboxMaps 11.11.0+
        locationUpdateCancellable = navigation?.locationMatching
            .throttle(for: .seconds(cameraUpdateInterval), scheduler: DispatchQueue.main, latest: true)
            .sink { [weak self] _ in
                guard let self = self else { return }
                let currentTime = Date().timeIntervalSince1970
                
                // Double-check throttling (belt and suspenders approach)
                if currentTime - self.lastCameraUpdateTime >= self.cameraUpdateInterval {
                    self.lastCameraUpdateTime = currentTime
                    // Camera will update naturally through the navigation system
                    // This throttle prevents excessive updates
                }
            }
    }
